...
class AudioDataset(Dataset):
    def __init__(self, df, resize, train_mode=True):
        ...
        # 定义训练时的增强操作
        self.time_shift = torchaudio.transforms.TimeMasking(time_mask_param=80)
        self.freq_mask = torchaudio.transforms.FrequencyMasking(freq_mask_param=80)
    ...
    def __getitem__(self, idx):
        ...
        waveform, sample_rate = torchaudio.load(
            self.audio_paths[idx], backend="soundfile"
        )

        # if self.train_mode:
        #     # 随机时移
        #     shift = int(
        #         torch.randint(-sample_rate // 10, sample_rate // 10, (1,)).item()
        #     )
        ...
        # 添加频谱增强
        if self.train_mode:
            mel_spectrogram = self.time_shift(mel_spectrogram)
...
def main():
    ...
    optimizer = optim.Adam(
        model.parameters(), lr=run.config.learning_rate, weight_decay=1e-5
    )
